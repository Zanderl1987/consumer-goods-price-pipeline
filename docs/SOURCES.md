...
| **USDA ERS specialty crops (Fruit & Tree Nuts / Vegetables & Pulses)** | Keyless CSVs — https://www.ers.usda.gov/data-products/fruit-and-tree-nuts-data/trade-and-prices-by-category-and-commodity and .../vegetables-and-pulses-data/... | Monthly CPI + PPI price indexes and average retail prices for selected specialty crops (grapes, oranges, avocados, potatoes ...), 2000-present; optional 95-127 MB monthly import/export trade CSVs by partner country | Monthly (~mid-month) | Download links carry a rotating `?v=NNNNN` cache-buster — request the bare `/media/{id}/price-index-data.csv` URL instead. **Correction (verified live 2026-08-24): both media IDs (6476 fruit, 5629 veg) served byte-identical price-index files** even though the pages list them separately; file covers both fruit AND vegetable series, so both tables land with identical content until ERS fixes/splits them. **Schema change (verified live 2026-09-05): the price-index CSVs are now tidy long format (`seriesType`, `seriesID`, `seriesName`, `year`, `monthNumber`, `unit`, `value`), 31,423 rows back to 2000, one download**; pipeline's old wide-pivot parser was updated to the new schema with a legacy fallback. `seriesID` is a stable per-series code (0 codes span multiple `seriesType`), so it feeds `curated.py`'s `(series_id, date)` natural key. **PIPELINE: ers_specialty_crops (built 2026-08-24, schema-fix 2026-09-05)** |
| **FEWS NET Data Warehouse** | Keyless REST — `https://fdw.fews.net/api/marketpricefacts/?dataset=FEWS_NET_Staple_Food_Price_Data&country=<ISO>&format=csv&fields=body` | Market-level retail/wholesale food prices across FEWS NET's monitored countries (63-col long-format export incl. CPC v2 product codes, unit, currency, exchange rate, common-currency price, period-over-period pct changes, lat/lon per market). Country-list discovered from `https://fdw.fews.net/api/market.json` (metadata renderer). Verified live 2026-09-05: Kenya 1,464 rows / 6 products (2005→2026-05), Ethiopia 110,783 rows / 22 markets / 20 products / Retail+Wage (2019→2026-05), full dataset ~267MB across ~90 candidate countries | Varies per market (weekly/monthly; `period_date` mostly month-end) | **GO (unblocked 2026-09-05) — but use the website export endpoint, NOT the raw renderers.** The warehouse's `*facts`/observation renderer family hangs indefinitely (500s/timeouts on every format, filter, and pagination variant; re-verified 2026-08-24 → 2026-09-05), INCLUDING the unscoped whole-dataset query (one 200 at ~267MB, then 500s on every retry). The same endpoint used behind the site's "Download File" buttons works reliably when scoped to ONE country (`fields=body` = full schema; `fields=website` is the site's tiny default table and is NOT used). Pipeline fetches per country with retry/backoff and skips countries with no observations; no key/quota. **Attribution required by FEWS NET's data use & attribution policy (https://help.fews.net/fdp/data-and-information-use-and-attribution-policy) — APIs explicitly permitted, redistribution permitted with attribution.** **PIPELINE: fews_net (built 2026-08-24, parked 2026-09-05, unblocked + rebuilt 2026-09-05)** |
...
| **Kroger catalog sweep additions (verified live 2026-08-25)** | same free API | Term-less listing is REJECTED on Certification (`PRODUCT-2016`: term or productId required) so whole-catalog walks are impossible; pagination works via `filter.start` (meta reports total); `filter.fulfillment` returns zero rows and items[].fulfillment carries availability booleans only, never per-channel prices -> no fulfillment price-spread table possible here | On-demand | **PIPELINE: kroger_catalog (built + activated 2026-08-25): ~46 broad terms x full pagination per working store, channel flags kept as columns; dead/test stores probed and skipped** |
...
| EC AgriFood fruit&veg supply-chain prices (api.tech.ec.europa.eu/agrifood) | **NO-GO / pending (probed 2026-08-24): endpoint paths undocumented** — the base returns 404 on every obvious path (`/`, `/api`, `/swagger`, `/openapi.json`), and agridata.ec.europa.eu is a Qlik Sense Hub whose portal HTML exposes no api.tech.ec.europa.eu resource references. Needs manual discovery (browser devtools against a live dashboard view) before a pipeline can be built |

## Recommendations

**Status as of 2026-08-24: every keyless source in this file is built**
(newly added: USDA ERS specialty crops + FEWS NET, both live-verified
2026-08-24; see their rows for caveats).
...
Kroger and Best Buy are also built and wired; Kroger is keyed and live
(kroger_products since 2026-08-04, kroger_catalog sweep added 2026-08-25).
Best Buy still SKIPs until a key is registered.

**Still open:** Best Buy is **BLOCKED** (2026-08-25): developer.bestbuy.com
does not allow free API key registration with Gmail addresses, so
`bestbuy_products` cannot be activated with available accounts; the code
stays wired and SKIPs cleanly. eBay (needs a
