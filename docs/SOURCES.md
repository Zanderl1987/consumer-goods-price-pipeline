...
| **FEWS NET Data Warehouse** | Keyless REST — https://fdw.fews.net/api/marketpricefacts.csv?fields=simple | Market-level retail/wholesale food prices across ~20 monitored countries (Ethiopia back to the 1990s), CPC v2 product codes, lat/lon per market | Varies per market (weekly/monthly) | **NO-GO (blocked upstream, re-confirmed 2026-09-05): the entire `*facts`/observation renderer family hangs indefinitely** — `marketpricefacts`, `marketprice`, `marketproduct`, `price`, `datapoint`, `datapointfacts`, `priceindexvaluefacts`, `tradeflowquantityvaluefacts`, `cropproductionfacts`, `commoditybalancefacts`, `exchangeratevaluefacts` all timeout at >45-300s in every format (json/csv), filter, and pagination variant incl. `Range` headers and `limit<=1`; only `priceratiovaluefacts` returns (empty `[]`). Metadata resources still respond (`/api/` 141 resources, `market` ~846KB/57k markets, `pricedataset`). Reproduced from probing 2026-08-24 → 2026-09-05, so this is a systemic server-side renderer outage, not transient. Pipeline `fews_net` is built defensively (retries + writes nothing on failure) and **parked** until FEWS NET fixes the renderer or publishes an alternate bulk export (e.g. fews.net HTML price pages would be a separate, re-vetted source). \(\)**PIPELINE: fews_net (built 2026-08-24, parked 2026-09-05)** |
