...
| **FEWS NET Data Warehouse** | Keyless REST — https://fdw.fews.net/api/marketpricefacts.csv?fields=simple | Market-level retail/wholesale food prices across ~20 monitored countries (Ethiopia back to the 1990s), CPC v2 product codes, lat/lon per market | Varies per market (weekly/monthly) | **Flaky live 2026-08-24**: resource list + earlier probe returned 200, but every data query during probing 502'd or read-timed out at <=300s (json or csv, limit<=5, date-filtered). Documented `?limit=&offset=` pagination unconfirmed on the CSV renderer; pipeline filters by `start_date` for incremental instead. Failed runs write nothing (idempotent). **PIPELINE: fews_net (built 2026-08-24)** |
